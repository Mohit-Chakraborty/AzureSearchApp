...
namespace AzureSearchApp
{
    ...
    {
        public static List<Hotel> GenerateHotels(int count, bool smallDocumentSize)
        {
            List<Hotel> hotels = new(count);

            for (int i = 0; i < count; i++)
            {
                hotels.Add(CreateHotel(i, smallDocumentSize));
            }

            return hotels;
        }

        private static Hotel CreateHotel(int id, bool smallDocumentSize)
        {
            Hotel hotel = new()
            {
                HotelId = id.ToString()
            ...
            if (smallDocumentSize)
            {
                ...
                hotel.Address = new HotelAddress()
                {
